...
@:dox(hide) class ApplicationMain {
	...
	public static function main () {
		...
		#if (js && html5)
		#if (munit || utest)
		lime.system.System.embed ("HelloWorld", null, 960, 640);
		#end
		#else
		create (null);
		#end
	...
	public static function create (config):Void {
		...
		app.meta["build"] = "1";
		app.meta["company"] = "Bowler Hat LLC";
		app.meta["file"] = "HelloWorld";
		app.meta["name"] = "Feathers | Hello World";
		app.meta["packageName"] = "com.feathersui.examples.HelloWorld";
		app.meta["version"] = "2.2.0";
		...
		app.window.context.attributes.background = 4866359;
	...
	macro public static function getEntryPoint () {
		...
		switch (Context.follow (Context.getType ("HelloWorld"))) {
			...
			case TInst (t, params):
				...
				if (hasMain) {
					...
					return Context.parse ("@:privateAccess HelloWorld.main ()", Context.currentPos ());
					...
					Context.fatalError ("Main class \"HelloWorld\" has neither a static main nor a constructor.", Context.currentPos ());
					
				}
				
			default:
				
				Context.fatalError ("Main class \"HelloWorld\" isn't a class.", Context.currentPos ());
...
@:keep @:dox(hide) class DocumentClass extends HelloWorld {}
